...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc"
   ...
   <p:input port="resource-names">
      <p:inline>
         <download path="https://github.com/usnistgov/OSCAL/releases/download/v1.1.2">
            <resource destination="lib">oscal_catalog_schema.xsd</resource>
            <resource destination="lib">oscal_complete_schema.xsd</resource>
         </download>
      </p:inline>
   </p:input>
   ...
   <p:for-each message="{$prefix} Saving resources in expressed target destinations ...">
      ...
      <p:store message="{$prefix} ... saving { $destination ! (. || '/')}{ $my-name }" href="{ ($destination,$my-name) => string-join('/')}"/>
   </p:for-each>
