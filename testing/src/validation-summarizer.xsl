...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="NOMINALLY-VALID/document[@SCHEMA-VALIDATION-STATUS='INVALID']" priority="10">
      <finding href="{@href}" status="failed" found="invalid">VALIDATION ANOMALY: a document supposed to be valid is found to be INVALID - { @VALIDATION-MODE } validation using schema { @SCHEMA }</finding>
   </xsl:template>
   
   <xsl:template match="NOMINALLY-INVALID/document[not(@SCHEMA-VALIDATION-STATUS='INVALID')]" priority="11">
      <finding href="{@href}" status="failed" found="valid">VALIDATION ANOMALY: a document supposed to be invalid is found to be VALID - { @VALIDATION-MODE } validation using schema { @SCHEMA }</finding>
   </xsl:template>
   
   <xsl:template match="document[@SCHEMA-VALIDATION-STATUS='INVALID']" priority="3">
      <finding href="{@href}" status="confirmed" found="invalid">CONFIRMED (invalid)</finding>
   </xsl:template>
   
   <xsl:template match="document">
      <finding href="{@href}" status="confirmed" found="valid">CONFIRMED (valid)</finding>
   </xsl:template>
